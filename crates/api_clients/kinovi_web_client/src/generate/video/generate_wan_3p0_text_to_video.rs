//! Wan 3.0 text-to-video through the Kinovi consumer workflow endpoint.
...
pub use crate::requests::workflow_run_task::workflow_run_task::WorkflowRunTaskResponse as GenerateWan3p0TextToVideoResponse;

pub struct GenerateWan3p0TextToVideoArgs<'a> {
  pub request: GenerateWan3p0TextToVideoRequest,
  pub session: &'a KinoviWebSession,
  pub maybe_host_override: Option<KinoviHost>,
}

#[derive(Clone, Debug)]
pub struct GenerateWan3p0TextToVideoRequest {
...
impl Default for GenerateWan3p0TextToVideoRequest {
  fn default() -> Self {
    Self {
      prompt: String::new(),
...
impl KinoviCostCalculatorTrait for GenerateWan3p0TextToVideoRequest {
  ...
  fn calculate_costs(&self, tier: KinoviPricingTier) -> Self::Cost {
    Wan3p0Model::TextToVideo.calculate_costs(self.maybe_output_resolution, self.duration_seconds, tier)
  }
}

impl GenerateWan3p0TextToVideoRequest {
...
pub async fn generate_wan_3p0_text_to_video(
  args: GenerateWan3p0TextToVideoArgs<'_>,
) -> Result<GenerateWan3p0TextToVideoResponse, KinoviWebError> {
