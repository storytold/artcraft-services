//! Wan 3.0 image-to-video through the Kinovi consumer workflow endpoint.
...
pub use crate::requests::workflow_run_task::workflow_run_task::WorkflowRunTaskResponse as GenerateWan3p0ImageToVideoResponse;

pub struct GenerateWan3p0ImageToVideoArgs<'a> {
  pub request: GenerateWan3p0ImageToVideoRequest,
  pub session: &'a KinoviWebSession,
  pub maybe_host_override: Option<KinoviHost>,
}

#[derive(Clone, Debug)]
pub struct GenerateWan3p0ImageToVideoRequest {
...
impl Default for GenerateWan3p0ImageToVideoRequest {
  fn default() -> Self {
    Self {
      prompt: String::new(),
...
impl KinoviCostCalculatorTrait for GenerateWan3p0ImageToVideoRequest {
  ...
  fn calculate_costs(&self, tier: KinoviPricingTier) -> Self::Cost {
    Wan3p0Model::ImageToVideo.calculate_costs(self.maybe_output_resolution, self.duration_seconds, tier)
  }
}

impl GenerateWan3p0ImageToVideoRequest {
...
pub async fn generate_wan_3p0_image_to_video(
  args: GenerateWan3p0ImageToVideoArgs<'_>,
) -> Result<GenerateWan3p0ImageToVideoResponse, KinoviWebError> {
