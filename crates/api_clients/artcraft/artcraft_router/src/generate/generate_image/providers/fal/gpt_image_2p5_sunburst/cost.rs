...
/// Cost state for Fal GPT Image 2.5 Sunburst. Delegates to `fal_client`'s calculator, which
...
#[derive(Clone, Debug)]
pub struct FalGptImage2p5SunburstCostState {
  cost_in_usd_cents: u64,
}

impl FalGptImage2p5SunburstCostState {
  pub fn from_request(request: &FalGptImage2p5SunburstRequestState) -> Self {
    let cost_in_usd_cents = match request {
      FalGptImage2p5SunburstRequestState::TextToImage(request) => request.calculate_cost_in_cents(),
      FalGptImage2p5SunburstRequestState::EditImage(request) => request.calculate_cost_in_cents(),
    };
