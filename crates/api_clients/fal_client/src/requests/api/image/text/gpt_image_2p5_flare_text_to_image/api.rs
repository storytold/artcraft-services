...
#[derive(Clone, Debug)]
pub struct GptImage2p5FlareTextToImageRequest {
  pub params: GptImage2p5TextToImageParams,
}

impl FalEndpoint for GptImage2p5FlareTextToImageRequest {
  const ENDPOINT: &str = "openai/gpt-image-2.5/flare/text-to-image";
...
mod tests {
  ...
  fn endpoint() {
    ...
    assert_eq!(GptImage2p5FlareTextToImageRequest::get_endpoint(), "openai/gpt-image-2.5/flare/text-to-image");
  }

  #[test]
  fn to_raw_request_delegates_to_params() {
    let request = GptImage2p5FlareTextToImageRequest {
